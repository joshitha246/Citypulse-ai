  ...
  if (query.includes('deploy') || query.includes('plan') || query.includes('emergency')) {
    ...
    return {
      html: `
        ...
        <p style="margin-top:8px;">Based on current incident analysis:</p>
        <p class="metric-line">🔴 Critical incidents: <strong>${criticalIncs.length}</strong></p>
        <p class="metric-line">🟠 High priority: <strong>${highIncs.length}</strong></p>
        <p style="margin-top:12px;"><strong>Recommended Actions:</strong></p>
        <p class="metric-line">1. 🚒 Deploy Fire Dept Unit 3 → ${criticalIncs[0]?.wardName || 'Mahadevapura'}</p>
        <p class="metric-line">2. 🚑 Position 2 ambulances → Shivajinagar</p>
        <p class="metric-line">3. 🚧 Road crew Alpha → ${analytics.topWards[0]?.[0] || 'Marathahalli'}</p>
        <p class="metric-line">4. 🌊 Storm water team → Koramangala underpass</p>
        <p class="metric-line">5. 🗑️ Sanitation sweep → Ward 14 Marathahalli</p>
        <p style="margin-top:12px;"><strong>⏱ Estimated full response time: 2.5 hours</strong></p>
        <p style="margin-top:8px;color:var(--accent-emerald);">✅ Plan generated by AI with 94.7% confidence.</p>
      `,
      actions: [
  ...
  if (query.includes('predict') || query.includes('forecast') || query.includes('next 24')) {
    return {
      html: `
        ...
        <p style="margin-top:8px;">Based on historical patterns, weather data, and current trends:</p>
        <p class="metric-line">📈 Expected incidents: <strong>22-28</strong> (moderate day)</p>
        <p class="metric-line">🌧️ Rain probability: <strong>65%</strong> (Marathahalli, Whitefield zones)</p>
        <p class="metric-line">🌊 Flooding risk: <strong>HIGH</strong> for Eastern zones</p>
        ...
        <p class="metric-line">• Schedule extra garbage collection routes for morning</p>
      `,
      actions: [
